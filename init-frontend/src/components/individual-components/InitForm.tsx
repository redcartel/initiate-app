...
const InitForm = ({ children, onSubmit, label, description, className }: { children: React.ReactNode, onSubmit: (e: React.FormEvent<HTMLFormElement>) => void, label: string, description: string, className?: string }) => {
    ...
    return (
        <Form onSubmit={(e) => {
            ...
            onSubmit(e);
            return false;
        }} className={`flex flex-col flex-1 gap-2 p-1 py-6 m-1 border-2 border-blue-200 rounded-lg bg-blue-50 ${className}`} aria-labelledby={id} aria-describedby={`${id}-description`}>
