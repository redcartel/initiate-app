...
export type CGButtonProps = ButtonProps & { theme: 'primary' | 'secondary' | 'tertiary' | 'action' | 'disabled' | 'destructive', hue?: 'light' | 'dark', size?: 'sm' | 'md' | 'lg' | 'xl', href?: string }

export const CGButton = ({ children, theme = 'secondary', hue, size, href, ...props }: CGButtonProps) => {
    let themeClassName = `bg-${theme}-400 text-${theme}-900 hover:bg-${theme}-500 hover:text-${theme}-900 `

    if (hue === 'light') {
        themeClassName = `bg-${theme}-200 text-${theme}-600 hover:bg-${theme}-300 hover:text-${theme}-700 `
    } else if (hue === 'dark') {
        themeClassName = `bg-${theme}-800 text-${theme}-100 hover:bg-${theme}-700 hover:text-${theme}-50 `
    }
