...
export function sendHtml(path: string, res: Response) {
    ...
    if (htmlPath.endsWith('.html') || htmlPath.endsWith('.htm')) {
        ...
        html = html.replace("href='/html/", `href='${process.env.API_BASE ?? 'https://api.d20init.com'}/html/`);
        html = html.replace("href=\"/html/", `href="${process.env.API_BASE ?? 'https://api.d20init.com'}/html/`);
        html = html.replace("src='/html/", `src='${process.env.API_BASE ?? 'https://api.d20init.com'}/html/`);
        html = html.replace("src=\"/html/", `src="${process.env.API_BASE ?? 'https://api.d20init.com'}/html/`);
